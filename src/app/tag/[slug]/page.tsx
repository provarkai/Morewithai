...
import { getSites, getPublicArticles } from "@/lib/api";
...
export default function TagArchivePage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => getPublicArticles(siteId, { tag: slug, page, limit: 12 }),
    enabled: !!siteId,
  });
  ...
  const siteName = sites?.[0]?.name || "MoreWithAI";
  const tagName = slug
    .split("-")
        ...
        <div className="mx-auto max-w-3xl px-4 py-8">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold">
              <Hash className="mr-1.5 inline-block size-7 align-text-bottom" />
              {tagName}
            </h1>
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
              {articles.map((article: any) => (
                <article
                  key={article.id}
                  className="group border-b pb-6 last:border-0"
                >
